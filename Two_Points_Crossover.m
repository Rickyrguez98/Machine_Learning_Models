...
for n=1:100 %Iteration or generations 
    ...
    % Two point crossover
    for k=1:np/4
        cp1 = randi([2, nbits-2]); % 1st crossover point
        cp2 = randi([cp1, nbits-1]); % 2nd crossover point
        binson(2*k-1,:)= [binparents(2*k-1, 1:cp1) binparents(2*k, cp1+1:cp2) binparents(2*k-1, cp2+1:nbits)];
        binson(2*k,:)= [binparents(2*k, 1:cp1) binparents(2*k-1, cp1+1:cp2) binparents(2*k, cp2+1:nbits)];
